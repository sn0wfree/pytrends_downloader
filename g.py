...
pytrends = TrendReq(hl='en-US', tz=360, timeout=(10, 25), proxies='', retries=2,
...
async def greet_world(req, resp, *, greeting):
    ...
    settings = req.params
    print(settings)
    # settings = await req.media(format='json')
    # print(settings)

    default_parameters = dict(cat=12, timeframe='all', geo='', gprop='')
    settings.update(default_parameters)

    resp.media = get_pytrends(**settings)
